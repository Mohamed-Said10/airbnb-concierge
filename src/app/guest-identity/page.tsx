          ...
          return (
            <li key={step} className="flex-1 flex items-center">
              <div className="flex flex-col items-center w-full">
                ...
                <span className={`mt-2 text-xs font-medium text-center ${active ? 'text-blue-600' : done ? 'text-gray-700' : 'text-gray-400'}`}>
                  {labels[step]}
                </span>
              </div>
              {i < STEPS.length - 1 && (
    ...
    <div>
      ...
      {preview ? (
        ...
        <div onClick={() => inputRef.current?.click()}
          ...
          <p className="text-sm text-gray-600"><span className="font-semibold text-blue-600">{clickLabel}</span> {dragLabel}</p>
...
const GuestIdentityPage = () => {
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white flex items-center justify-center px-4 pt-20">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white pt-20 pb-16 px-4">
      <div className="max-w-2xl mx-auto">

        {/* Header */}
        <div className="text-center mb-10">
          <span className="inline-block bg-blue-100 text-blue-700 text-xs font-semibold tracking-wider uppercase px-3 py-1 rounded-full mb-4">
            KoziBnB
          </span>
          ...
          <form onSubmit={handleSubmit} noValidate>
              ...
              <div className="space-y-6">
                ...
                {form.travelers.map((traveler, idx) => (
                  <div key={idx} className="border border-gray-200 rounded-xl overflow-hidden">
                    {/* Card header */}
                    <div className={`flex items-center justify-between px-4 py-3 ${idx === 0 ? 'bg-blue-600' : 'bg-gray-700'}`}>
                ...
                <button type="button" onClick={addTraveler}
                  className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-blue-300 text-blue-600 hover:border-blue-500 hover:bg-blue-50 rounded-xl py-3 text-sm font-semibold transition-colors">
            ...
            {step === 'upload' && (
              <div className="space-y-8">
                ...
                {form.travelers.map((traveler, idx) => (
                  <div key={idx} className="border border-gray-200 rounded-xl overflow-hidden">
                    <div className={`px-4 py-2.5 ${idx === 0 ? 'bg-blue-600' : 'bg-gray-700'}`}>
                      <span className="text-sm font-semibold text-white">
                        {idx === 0 ? gi.multi.mainTraveler : `${gi.multi.traveler} ${idx + 1}`}
                        {travelerName(traveler) ? ` — ${travelerName(traveler)}` : ''}
                      </span>
            ...
            {step === 'signature' && (
              <div className="space-y-6">
                ...
                <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                  <p className="text-sm text-blue-700">{gi.signatureStep.submitInstruction}</p>
                </div>
              </div>
            )}
            ...
            <div className="flex justify-between mt-8 pt-6 border-t border-gray-100">
              ...
              {step !== 'signature' ? (
                ...
                <button type="submit" disabled={submitting}
                  className="px-6 py-2.5 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-70 transition-colors flex items-center gap-2">
                  {submitting && (
...
const inputCls = (hasError: boolean) =>
  `mt-1 block w-full rounded-lg border px-3 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition
   ${hasError ? 'border-red-400 focus:ring-red-400' : 'border-gray-300 focus:border-blue-500'}`;
