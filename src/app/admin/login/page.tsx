  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profileError || profile?.role !== 'admin') {
        ...
        setError(french ? "Ce compte n'a pas d'accès administrateur." : 'This account does not have administrator access.');
        return;
      }
      window.location.replace('/admin');
    } catch {
      setError(french ? 'E-mail ou mot de passe incorrect.' : 'Email or password is incorrect.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <div className="bg-white rounded-xl shadow-lg p-5 sm:p-8 w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="mt-1 text-sm text-gray-500">{french ? "Tableau de bord d'administration" : 'Admin Dashboard'}</p>
        </div>
